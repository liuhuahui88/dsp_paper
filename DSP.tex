...
We solve this problem by
    ...
    3) claiming the DSP problem is a special case of the augmented GAP and deriving specialized strategy.
...
\item \textbf{Revenue}(i.e. gross income): the total amount of money(e.g. 50\$) earned from advertisers
                                           through either payment modes mentioned above.
...
\item claiming the DSP problem is a special case of the augmented GAP and deriving specialized strategy(Section \ref{Solution}).
\end{enumerate}
...
The profit is defined as $\sV(bp)=\sCPI{}WinProb_i(bp)-WinPrice_i(bp)$, so we have $\pprob=\sCPI$ and $\pprice=-1$.
