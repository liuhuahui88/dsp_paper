...
Fourth, the constraints are set on ad group level,
...
$\uff$ is the function family that $\forall f \in \uff$ is of the form
    $ \uf = \int_0^{bp} (\pprob + \pcost x)p(x)dx = \pprob Prob(bp) + \pcost Cost(bp) $.
\end{definition}
...
\begin{theorem}
Given $\forall f \in \uff$ with $\pprob > 0$ and $\pcost < 0$, we have $\argmax\limits_{bp} \uf = - \pprob / \pcost$.
\end{theorem}

\begin{theorem}
Given $\forall g,h \in \uff$ with common $p(x)$,
    we have $\frac{d^2h}{d^2g} = \frac{\pprob^g \pcost^h - \pcost^g \pprob^h}{(\pprob^g + \pcost^g bp)^3 p(bp)}$.
\end{theorem}
...
In certain applications, $Q_{ij}$ is the same for given $i$ and all $j$, then we have $j^* = \argmax\limits_j \sbp^*$.
For example, when maximizing profit under budget constraint in P4P mode,
    we have $\sF(bp) = \int_0^{bp} [(1-\salpha)\sCPI-x]p_i(x)dx$ and $Q_{ij}$ is always 1.
...
\For{$Impression_i \in \mathscr{I}$}
{
  ...
  {
    $\sF(bp; \valpha^*) = \int_0^{bp} (P_{ij}-Q_{ij}x)p_i(x)dx$

    $\sbp^* = \sbp(\valpha^*) = P_{ij}/Q_{ij}$
...
All ads are in P4P mode with budget constraints, while DSP is willing to maximize its profit.
