...
It's essential for DSP to find an optimal ad selection and bid price determination strategy
    which maximizes revenue, profit or performance under budget and ROI(Return On Investment) constraints
    in PFI(Pay For Impression) or PFU(Pay For Usage) mode.
We solve this problem by
    1) formalizing the DSP problem as a constrained optimization problem,
    2) proposing the augmented GAP(Generalized Assignment Problem) with general solution,
    3) and demonstrating the DSP problem is a special case of the augmented GAP and deriving specialized strategy.
...
\begin{enumerate}
\item \textbf{PFI}(Pay For Impression): the advertiser sets a CPI(Cost Per Impression)
    and pays DSP the CPI(e.g. 0.001\$) for each impression.
\item \textbf{PFU}(Pay For Usage): the advertiser sets a commission rate(e.g. 0.1)
    and pays DSP the total bidding cost(e.g. 100\$) plus the fraction of it as commission(e.g. 0.1*100\$=10\$).
\end{enumerate}

DSP is interested in optimizing one of the following objectives
\begin{enumerate}
\item \textbf{Revenue}(i.e. gross income): the total amount of money(e.g. 50\$) earned from advertisers
                                           through either payment mode mentioned above.
\item \textbf{Profit}(i.e. net income): the revenue(e.g. 50\$) minus the total bidding cost(e.g. 40\$) payed to AdX.
\item \textbf{Performance}: the total number of impressions or clicks delivered to advertisers.
\end{enumerate}

During the optimization, several constraints must be satisfied
\begin{enumerate}
\item \textbf{Budget Upper Bound}: the maximum amount of money the advertiser is willing to spend for a certain period of time
  (e.g. 100\$ per day).
\item \textbf{ROI Lower Bound}: the minimum value of ROI(Return On Investment)
  which is defined as, for DSP, the money earned from advertisers(i.e. revenue) over the bidding cost payed to AdX
  (e.g. ROI is 1.1 when DSP earns 110\$ and pays 100\$)
  and, for advertiser, the number of impressions or clicks(i.e. performance) over the money spent in DSP
  (e.g. ROI is 0.8 when advertiser spends 100\$ for 80 clicks).
\end{enumerate}

It's essential for DSP to find an optimal ad selection and bid price determination strategy
    which maximizes revenue, profit or performance under budget and ROI constraints in PFI or PFU mode.
We solve this problem by
\begin{enumerate}
\item formalizing the DSP problem as a constrained optimization problem(Section \ref{Formalization}),
\item proposing the augmented GAP(Generalized Assignment Problem) with general solution(Section \ref{AugmentedGAP}),
\item and demonstrating the DSP problem is a special case of the augmented GAP and deriving specialized strategy(Section \ref{Solution}).
\end{enumerate}
...
First, in PFI mode, the advertiser usually prefers to set CPC(Cost Per Click) rather than CPI
    and the DSP is supposed to transform CPC into CPI with predicted CTR(Click Through Rate).
However, CTR prediction is beyond the scope of this document and,
    without loss of generality, we assume that the CPI, if not specifically mentioned,
    is always explicitly provided in the rest of our discussion.
Second, in PFI mode, if all advertisers choose to set the same CPI or CPC,
    since the number of impressions or clicks is now proportional to the revenue,
    maximizing the revenue is equivalent to maximizing the performance.
Third, in PFU mode, the revenue is the total bidding cost plus the commission by definition,
    while the profit is exactly the commission which is proportional to the revenue.
As a result, given the fixed global commission rate, maximumization of revenue and profit are equivalent.
At last, the ROI lower bound for advertiser could also be interpreted as the averaged CPI or CPC upper bound
...
The above formalization might seem too abstract to capture the details of
    those concrete objectives and constraints discussed in Section \ref{Introduction}.
To make things clearer, we
\begin{enumerate}
\item derive the expected winning probability and cost under second price auction assumption(Section \ref{SecondPriceAuction}),
\item define the utility function family $\uff$ based on previous derivation(Section \ref{UtilityFunctionFamily}),
\item and show how to systematically encode those concrete objectives and constraints into above formalization
    by setting $B$ and choosing $V$ and $W$ from $\uff$(Section \ref{Objectives} \& \ref{Constraints}).
\end{enumerate}
...
Then the expected winning probability and cost for our DSP could be defined as follows.
As our $BidPrice$ goes infinit, we'll win $Impression_i$ with probability 1 and our cost must be the mean of $p_i(x)$.

\begin{alignat}{2}
WinProb_i(BidPrice)= & \int_0^{BidPrice} p_i(x) \mathrm{d} x \\
Cost_i(BidPrice)=    & \int_0^{BidPrice} x p_i(x) \mathrm{d} x
\end{alignat}

It is the non-negativity property of $p_i(x)$ and the integral forms of $WinProb_i(BidPrice)$ and $Cost_i(BidPrice)$
...
\begin{definition}
$\uff = \{ \uf = \pprob WinProb(bp) + \pprice Cost(bp) = \int_0^{bp} (\pprob + \pprice x)p(x)dx \} $
\end{definition}
...
The profit is defined as $\sV(bp)=\sCPI{}WinProb_i(bp)-Cost_i(bp)$, so we have $\pprob=\sCPI$ and $\pprice=-1$.
...
In PFI mode, DSP could set lower bound for ROI.
In PFU mode, advertisers could set upper bound for CPI and CPC.
...
The budget consumption is defined as $\sV(bp)=Cost_i(bp)$,
...
\cline{3-4}
             &              & $\pprob$              & $\pprice$      & \\
\hline
\mr{2}{PFI}  & Budget 1     & $\sCPI$               & 0              & $Budget^{(k)}$ \\
\cline{2-5}
             & ROI          & $-\sCPI$              & $ROI^{(k)}$    & 0 \\
\hline
\mr{3}{PFU}  & Budget 2     & 0                     & 1              & $Budget^{(k)}$ \\
\cline{2-5}
             & Average CPI  & $-CPI^{(k)}$          & 1              & 0 \\
\cline{2-5}
...
\section{Augmented GAP} \label{AugmentedGAP}
...
For example, when maximizing profit under budget constraint in PFI mode,
    we have $\sF(bp) = \int_0^{bp} [(1-\salpha)\sCPI-x]p_i(x)dx$ and $Q_{i,j}$ is always 1.
...
In $Case_3$, $Ad_3$ is in PFU mode with budget and average CPI constraints, and DSP prefers to maximize its commission.
...
With a well calibrated CPI predictor, the problem could be transformed equivalently into one in PFI mode.
...
All ads are in PFI mode with budget constraints, while DSP is willing to maximize its profit.
Besides, these ads share the universal constant CPI and are targeted respectively to overlaped sets of impressions.
